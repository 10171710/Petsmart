...
(function () {
  ...
  function render() {
    ...
    var setHtml = function (id, html) { var el = document.getElementById(id); if (el && html != null) el.innerHTML = html; };
    setHtml("blog-date", '<i class="fa-solid fa-calendar-days"></i> ' + esc(post.date));
    setHtml("blog-read", '<i class="fa-solid fa-clock"></i> ' + esc(post.readTime));
    setHtml("blog-comments", '<i class="fa-solid fa-comments"></i> ' + esc(post.comments));

    var author = post.author || {};
    var av = document.getElementById("blog-author-avatar");
    if (av) av.src = author.avatar || "";
    ...
    if (abAv) abAv.src = author.avatar || "";

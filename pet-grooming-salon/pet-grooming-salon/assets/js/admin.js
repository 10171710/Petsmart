...
(function () {
  ...
  try {
    ...
    if (_sess && _sess.role === "admin" && _sess.name) ADMIN_NAME = _sess.name;
  } catch (err) {}
  var ADMIN_INITIAL = (ADMIN_NAME || "A").trim().charAt(0).toUpperCase() || "A";
  ...
  <aside class="admin-sidebar" id="admin-sidebar">
    <a class="sidebar-brand" href="index.html">
      ...
      <div>
        <div class="bb-name">PawVille</div>
    ...
    <nav class="flex-grow-1">
      ...
      <ul class="nav flex-column">
        ...
        <li class="nav-item"><a class="nav-link" href="appointments.html" data-nav="appointments"><span class="nav-ico">📅</span> Appointments</a></li>
        <li class="nav-item"><a class="nav-link" href="customers.html" data-nav="customers"><span class="nav-ico">👥</span> Customers</a></li>
        <li class="nav-item"><a class="nav-link" href="messages.html" data-nav="messages"><span class="nav-ico">💬</span> Messages <span class="badge-clay bg-coral ms-auto" style="font-size:.72rem">4</span></a></li>
        <li class="nav-item"><a class="nav-link" href="analytics.html" data-nav="analytics"><span class="nav-ico">📈</span> Analytics</a></li>
      </ul>
      <div class="nav-section">General</div>
      <ul class="nav flex-column">
        <li class="nav-item"><a class="nav-link" href="#" data-nav="settings"><span class="nav-ico">⚙️</span> Settings</a></li>
        <li class="nav-item"><a class="nav-link" href="#" data-nav="help"><span class="nav-ico">❓</span> Help Center</a></li>      </ul>
    </nav>

    <div class="clay-inset p-3 mt-3">
      <div class="d-flex align-items-center gap-3">
        <div class="clay-avatar sm" style="display:flex;align-items:center;justify-content:center;font-weight:800;font-size:1rem">${ADMIN_INITIAL}</div>
        <div class="flex-grow-1">
          <div class="fw-bold" style="font-size:.9rem">${ADMIN_NAME}</div>
          <div class="text-secondary" style="font-size:.78rem">Administrator</div>
        </div>
        <a href="../index.html" title="View website" class="nav-icon-btn" style="width:34px;height:34px">↗</a>
      </div>
      <a href="#" class="btn-clay-outline btn-sm w-100 mt-3" data-logout>🚪 Log out</a>
    </div>
  </aside>
  `;
  ...
  <header class="admin-topbar" id="admin-topbar">
    ...
    <div class="ms-auto d-flex align-items-center gap-2">
      ...
      <button class="nav-icon-btn position-relative" type="button" id="admin-bell" title="Notifications" aria-label="Notifications">
        🔔
        <span class="position-absolute top-0 start-100 translate-middle badge rounded-pill" style="background:var(--pv-primary);font-size:.6rem">4</span>
      </button>
      <div class="dropdown">
        <a href="#" class="d-flex align-items-center gap-2 text-decoration-none" data-bs-toggle="dropdown" aria-expanded="false">
          <div class="clay-avatar sm" style="display:flex;align-items:center;justify-content:center;font-weight:800;color:var(--pv-primary);font-size:1rem">${ADMIN_INITIAL}</div>
          <span class="fw-bold d-none d-lg-inline" style="color:var(--pv-text)">${ADMIN_NAME}</span>
        </a>
        <ul class="dropdown-menu dropdown-menu-end" style="min-width:200px">
          <li><a class="dropdown-item" href="index.html">📋 My Dashboard</a></li>
          <li><a class="dropdown-item" href="../index.html">🌐 View Website</a></li>
          <li><hr class="dropdown-divider"></li>
          <li><a class="dropdown-item" href="#" data-logout>🚪 Logout</a></li>
        </ul>
      </div>
    </div>
  </header>
  ...
  function statusBadge(status) {
    var map = {
      pending: '<span class="badge-clay bg-amber">Pending</span>',
      confirmed: '<span class="badge-clay bg-teal">Confirmed</span>',
      completed: '<span class="badge-clay bg-violet">Completed</span>',
      cancelled: '<span class="badge-clay" style="color:var(--pv-muted)">Cancelled</span>'
    };
    return map[status] || map.pending;
  }

  var PET_EMOJI = { dog: "🐶", cat: "🐱", rabbit: "🐰", bird: "🐦" };
      ...
      '<tr data-id="' + a.id + '">' +
        ...
        '<td class="text-end"><div class="dropdown">' +
          ...
          '<ul class="dropdown-menu dropdown-menu-end">' +
            ...
            '<li><a class="dropdown-item" href="#" data-action="complete">✔ Complete</a></li>' +
            '<li><a class="dropdown-item text-danger" href="#" data-action="cancel">✕ Cancel</a></li>' +
          '</ul></div></td>' +
      '</tr>'
    );
  }
        ...
        ? done.map(function (a) {
            return (
              '<tr>' +
                ...
                '<td>' + a.date + '</td>' +
                '<td>' + statusBadge(a.status) + '</td>' +
                '<td class="text-end"><button class="btn-clay btn-sm" data-invoice="' + a.id + '">🧾 Invoice</button></td>' +
              '</tr>'
            );
          }).join("")
  ...
  /* ------------------------------------------------------------------
     Init (admin session guard)
  ------------------------------------------------------------------ */
  function init() {
    var _s = window.PawSession && PawSession.get();
    if (!_s || _s.role !== "admin") {
  ...
  function bindAdminUtility() {
    ...
    if (bell) bell.addEventListener("click", function () {
      PawPopup({
        icon: "🔔",
        title: "Notifications",
        content: "<ul style='margin:0'><li>3 new booking requests await confirmation</li><li>1 client message needs a reply</li><li>New review from Rachel H. — 5 paws ⭐</li></ul>",
        wide: true
      });
    });
    ...
    overlay.innerHTML =
      ...
      'font-family:\'Baloo 2\',sans-serif">' +
        ...
        '<form id="add-customer-form" style="margin-top:14px">' +
          ...
          '<div id="phone-err" style="display:none;color:#f0506e;font-size:.8rem;font-weight:700;margin-top:4px">Phone must be exactly 10 digits.</div></div>' +
  ...
  /* ------------------------------------------------------------------
     Logout (visible button + topbar menu) — clears the session
  ------------------------------------------------------------------ */
  function bindLogout() {
